...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        try:
            await db.execute("ALTER TABLE orders ADD COLUMN failure_reason TEXT")
        except aiosqlite.OperationalError as e:
            # SQLite raises this exact message when the column already exists.
            # Any other OperationalError is a real problem and should propagate.
...
async def deduct_balance(telegram_id: int, amount: int, tx_type: str, description: str) -> bool:
    ...
    Uses a conditional UPDATE so the balance check and the decrement happen
    in a single SQL statement. Prevents the TOCTOU race where two concurrent
    orders could both pass an external check and drive the balance negative.
    """
    if not isinstance(amount, int) or amount <= 0:
        raise ValueError(f"deduct_balance: amount must be a positive integer, got {amount!r}")
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
...
async def update_order(order_id: int, status: str, link: str = None, failure_reason: str = None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE orders SET status = ?, generated_link = ?, failure_reason = ? WHERE id = ?",
            (status, link, failure_reason, order_id),
        )
        await db.commit()


async def count_recent_failures_by_reason(user_id: int, reason: str, hours: int = 24) -> int:
    """Count this user's failed orders with the given failure_reason in the last N hours."""
    from datetime import timedelta
    cutoff = (datetime.now() - timedelta(hours=hours)).isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT COUNT(*) FROM orders "
            "WHERE user_id = ? AND status = 'failed' AND failure_reason = ? AND created_at >= ?",
            (user_id, reason, cutoff),
        ) as c:
...
async def get_stats():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("SELECT COUNT(*) FROM orders WHERE status = 'processing'") as c:
            processing_orders = (await c.fetchone())[0]
        async with db.execute("SELECT COUNT(DISTINCT user_id) FROM user_proxies") as c:
            proxy_users = (await c.fetchone())[0]
        async with db.execute(
            "SELECT COALESCE(failure_reason, 'UNKNOWN') AS reason, COUNT(*) AS cnt "
            "FROM orders WHERE status = 'failed' "
            "GROUP BY COALESCE(failure_reason, 'UNKNOWN') "
            "ORDER BY cnt DESC"
        ) as c:
            failure_rows = await c.fetchall()
...
async def list_user_proxies(user_id: int) -> list[str]:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT proxy_url FROM user_proxies WHERE user_id = ? ORDER BY created_at ASC",
            (user_id,),
        ) as cursor:
            return [r[0] for r in await cursor.fetchall()]


async def add_user_proxy_url(user_id: int, proxy_url: str) -> bool:
    """Add one proxy URL for a user. Returns True if newly inserted, False if duplicate."""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT OR IGNORE INTO user_proxies (user_id, proxy_url, created_at) VALUES (?, ?, ?)",
            (user_id, proxy_url, datetime.now().isoformat()),
        )
        await db.commit()
        return cursor.rowcount == 1


async def remove_user_proxy_url(user_id: int, proxy_url: str | None = None) -> int:
    """Remove a specific proxy or (when proxy_url is None) all proxies for the user.
    Returns number of remaining proxies."""
    async with aiosqlite.connect(DB_PATH) as db:
        if proxy_url is None:
            await db.execute("DELETE FROM user_proxies WHERE user_id = ?", (user_id,))
        else:
            await db.execute(
                "DELETE FROM user_proxies WHERE user_id = ? AND proxy_url = ?",
                (user_id, proxy_url),
            )
        await db.commit()
        async with db.execute(
...
async def get_random_user_proxy(user_id: int) -> str | None:
    """Return one randomly chosen proxy URL for the user, or None if none configured."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT proxy_url FROM user_proxies WHERE user_id = ? "
