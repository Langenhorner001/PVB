...
DEFAULT_EXCLUDE_SUFFIXES = {
    ".env",
    ".db",        # Never overwrite remote bot_data.db (preserves user balances/history)
    ".db-journal",
    ".db-wal",
    ".db-shm",
    ".pyc",
...
def sudo_prefix() -> str:
    sudo_password = env("EC2_SUDO_PASSWORD") or env("EC2_PASSWORD")
    if sudo_password:
...
def sync_service_file(client, sftp, remote_root: str) -> bool:
    ...
    service_name = env("EC2_SERVICE", "pixel-bot")
    start_cmd = env(
        "EC2_START_COMMAND",
    ...
    def _esc_env_value(v: str) -> str:
        # Inside Environment="KEY=VAL", escape \ and " so a stray quote in
        # PAYMENT_ACCOUNT_NAME etc. cannot break the unit file.
    ...
    for k in extra_env_keys:
        v = env(k)
        if v:
            extra_env_lines += f'Environment="{k}={_esc_env_value(v)}"\n'

    # Pin Playwright browser cache inside the deploy dir so it lives in
    # a path the systemd hardening (ProtectHome=read-only / ReadWritePaths)
    # allows writes to.
    pw_browsers = f"{remote_root}/.cache/ms-playwright"
    ...
    content = (
        ...
        f'Environment="BOT_TOKEN={_esc_env_value(bot_token)}"\n'
        f'Environment="ADMIN_ID={_esc_env_value(admin_id)}"\n'
        f'Environment="PLAYWRIGHT_BROWSERS_PATH={pw_browsers}"\n'
        f"{extra_env_lines}"
    ...
    # `sudo mv` from /tmp inherits world-readable perms from sftp upload —
    # explicit chmod 600 prevents BOT_TOKEN from leaking to other users on the box.
    sudo = sudo_prefix()
    cmd = (
        f"{sudo}mv {shlex.quote(tmp_path)} {shlex.quote(service_path)} "
        f"&& {sudo}chmod 600 {shlex.quote(service_path)} "
        f"&& {sudo}chown root:root {shlex.quote(service_path)} "
        f"&& {sudo}systemctl daemon-reload"
    )
    stdin, stdout, stderr = client.exec_command(cmd, get_pty=False)
    stdout.channel.recv_exit_status()
    log("Service file updated (chmod 600) with fresh BOT_TOKEN & ADMIN_ID.", GREEN)
    return True


def install_remote_deps(client, remote_root: str) -> None:
    req_path = f"{remote_root}/requirements.txt"
    pip_bin = f"{remote_root}/venv/bin/pip"
    pip_install = (
        f"if [ -x {shlex.quote(pip_bin)} ]; then "
        f"{shlex.quote(pip_bin)} install -r {shlex.quote(req_path)} -q; "
...
def install_remote_playwright(client, remote_root: str) -> None:
    """Install/refresh Playwright Chromium browser on the remote.

    Browsers are pinned to ``$remote_root/.cache/ms-playwright`` so the
    systemd unit's ProtectHome=read-only does not lock Chromium out at runtime.
    """
    if truthy(env("DEPLOY_SKIP_PLAYWRIGHT")):
        log("DEPLOY_SKIP_PLAYWRIGHT=1 → Playwright install skip.", YELLOW)
        return

    pw_bin = f"{remote_root}/venv/bin/playwright"
    pw_path = f"{remote_root}/.cache/ms-playwright"
    cmd = (
        f"if [ -x {shlex.quote(pw_bin)} ]; then "
        f"mkdir -p {shlex.quote(pw_path)} && "
        f"PLAYWRIGHT_BROWSERS_PATH={shlex.quote(pw_path)} "
        f"{shlex.quote(pw_bin)} install chromium >/dev/null 2>&1 && echo OK || echo FAIL; "
        "else echo SKIP; fi"
    )
    log("Playwright Chromium install/refresh...", CYAN)
    stdin, stdout, stderr = client.exec_command(cmd, get_pty=False)
    stdout.channel.recv_exit_status()
    out = (stdout.read().decode(errors="replace") or "").strip()
    if out == "OK":
        log("Playwright Chromium ready.", GREEN)
    elif out == "SKIP":
        log("Playwright binary not found in venv — pip install ke baad re-run karein.", YELLOW)
    else:
        log("Playwright install fail (non-fatal). setup.sh re-run karein.", YELLOW)


def restart_remote_service(client, service_name: str) -> bool:
    log(f"Bot restart kar raha hoon: {service_name}", CYAN)
    stdin, stdout, stderr = client.exec_command(
        f"{sudo_prefix()}systemctl restart {shlex.quote(service_name)}",
        get_pty=False,
    )
...
def deploy_to_ec2() -> bool:
    ...
    password = env("EC2_PASSWORD") or env("VPS_PASSWORD")
    ...
    try:
        ...
        log(
            "paramiko library nahi mili. Run: "
    ...
    pkey = None
    has_explicit_key = bool(env("EC2_SSH_KEY") or env("EC2_SSH_KEY_PATH"))
    has_local_key = (ROOT / "ec2_key.pem").exists() or (ROOT / "ec2_key.ppk").exists()
    if has_explicit_key or (has_local_key and not password):
        try:
            pkey = load_ssh_key()
        except Exception as exc:
        ...
        log("SSH auth missing. Set EC2_PASSWORD for VPS password auth, or EC2_SSH_KEY/EC2_SSH_KEY_PATH for key auth.", RED)
        return False
    ...
    try:
        ...
        install_remote_playwright(client, remote_root)
